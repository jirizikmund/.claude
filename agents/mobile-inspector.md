...
- **Before writing/updating ANY file**, always READ the existing file first to see its current content
...
### CRITICAL: Rules for Updating `index.md`
- **ALWAYS read `index.md` FIRST** before making any changes
- **NEVER overwrite or replace the entire file** — only APPEND new rows to existing tables
- When adding a new screen, insert a new row into the "Known Screens" table — do NOT remove existing rows
- When adding a new screenshot, insert a new row into the "Screenshots" table — do NOT remove existing rows
- Preserve ALL existing content, formatting, headers, and metadata
- If you cannot edit the file (no Edit tool), return the exact rows to be added and let the orchestrator handle the file update
